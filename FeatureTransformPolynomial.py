...
class PolynomialFeatureTransform:
    ...
    def __init__(self, order=1, input_dim=7, input_feature_names=None):
    ...
    def transform(self, x_ND):
        ...
        phi_NM = np.zeros((N, self.output_dim), dtype=np.float64)

        phi_NM[:, 0] = 1.0
        m_dim = 1
        for P in range(1, self.order+1):

            x_ND_np = x_ND.values  # Convert to NumPy array
            phi_NM[:, m_dim:(m_dim + D)] = np.power(x_ND_np, P)
            m_dim += D
